...
void LibStrikerProvider::update(LibStriker& libStriker)
{
  ...
  libStriker.GoalPostTarget = [this]() -> Vector2f {
    return GoalPostTarget();
  };
  libStriker.areThereOpponentsNearTarget = [this](const Vector2f& point, float radius) -> bool{
    return areThereOpponentsNearTarget(point, radius);
  };
}
...
Vector2f LibStrikerProvider::GoalPostTarget() const
{
 Vector2f target;
  //se palla esterna a porta 
 if(theFieldBall.recentBallPositionOnField().y() > theFieldDimensions.yPosLeftGoal || theFieldBall.recentBallPositionOnField().y() < (theFieldDimensions.yPosRightGoal - 130.f) ){
 	 ...
 	 if (theFieldBall.recentBallPositionOnField().y() > theFieldDimensions.yPosLeftGoal){
 	 	//esterno sx
 	 	//tira verso il centro dell'area piccola
 	 	target = Vector2f(theFieldDimensions.xPosOpponentGroundLine - 400.f, theFieldDimensions.yPosLeftGoal - 100.f);
		//se ci sono avversari sulla traiettoria
		Pose2f nearestOpp = theLibObstacles.nearestOpponentToBall();
		if(nearestOpp.translation.x() < theFieldDimensions.xPosOpponentGroundLine && nearestOpp.translation.x() >= (theFieldDimensions.xPosOpponentGroundLine - 400.f) && abs(nearestOpp.translation.y()) < abs(theFieldBall.recentBallPositionOnField().y()) && abs(nearestOpp.translation.y()) >= (abs(theFieldDimensions.yPosLeftGoal) - 200.f)){
 	 			//se la palla è vicina all'avversario aggiralo		
 	 			if (abs(theFieldBall.recentBallPositionOnField().y() - nearestOpp.translation.y()) <= 600){
 	 				target = Vector2f(nearestOpp.translation.x() - 600.f, nearestOpp.translation.y());
 	 			}
 	 	}
 	 	...
 	 	//esterno dx
 	 	//tira verso il centro dell'area piccola
 	 	target = Vector2f(theFieldDimensions.xPosOpponentGroundLine - 400.f, theFieldDimensions.yPosRightGoal - 50.f);
 	 	//se ci sono avversari sulla traiettoria
 	 	Pose2f nearestOpp = theLibObstacles.nearestOpponentToBall();
 	 	if(nearestOpp.translation.x() < theFieldDimensions.xPosOpponentGroundLine && nearestOpp.translation.x() >= (theFieldDimensions.xPosOpponentGroundLine - 400.f) && abs(nearestOpp.translation.y()) < abs(theFieldBall.recentBallPositionOnField().y()) && abs(nearestOpp.translation.y()) >= abs(theFieldDimensions.yPosRightGoal)){
 	 			//se la palla è vicina all'avversario aggiralo		
 	 			if (abs(theFieldBall.recentBallPositionOnField().y() - nearestOpp.translation.y()) <= 600){
 	 				target = Vector2f(nearestOpp.translation.x() - 600.f, nearestOpp.translation.y());
 	 			}
 ...
 //se palla in corrispondenza di palo
 } else if (abs(theFieldBall.recentBallPositionOnField().y() - theFieldDimensions.yPosLeftGoal + 150.f) <= 150.f || abs(theFieldBall.recentBallPositionOnField().y() - theFieldDimensions.yPosRightGoal) <= 170.f){
 	printf("a filo\n");
 	...
 	//palo sx
 	if (abs(theFieldBall.recentBallPositionOnField().y() - theFieldDimensions.yPosLeftGoal + 150.f) <= 150.f){
 		//se il palo è libero
 		
 		if(!areThereOpponentsNearTarget(postleft, 400.f)){
 			printf("palo libero\n");
 			//tira subito dopo il palo in porta
 			target = Vector2f(theFieldDimensions.xPosOpponentGoal, postleft.y() - 300.f);
 		}else{	
 			//tira verso dove stai guardando in porta
 			target = Vector2f(goalTarget(true, false));

 			printf("palo occupato\n");
 		}
 	//palo dx
 	 } else {
 	 	//se il palo è libero
 	 	if(!areThereOpponentsNearTarget(postright, 350.f)){
 	 		printf("palo libero dx\n");
 			target = Vector2f(theFieldDimensions.xPosOpponentGoal, -300.f);
 		}else{	
 			//tira verso dove stai guardando in porta
 			target = Vector2f(goalTarget(true, false));
 			
 			printf("palo occupato dx\n");
 		}
 	}
...
//ritorna se ci sono avversari vicini al target passato con "point" in coordinate globali 
bool LibStrikerProvider::areThereOpponentsNearTarget(const Vector2f& point, float radius) const {
