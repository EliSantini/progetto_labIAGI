...
void LibObstaclesProvider::update(LibObstacles& libObstacles)
{
  ...
  */
  ...
  libObstacles.nearestOpponentToBall = [this]() -> Pose2f {
    return nearestOpponentToBall();
  };
}
...
//trova l'avversario più vicino alla palla in coordinate globali
Pose2f LibObstaclesProvider::nearestOpponentToBall() const {
  Pose2f nearest = Pose2f(9000.f,6000.f);
  for(const auto& obs : theTeamPlayersModel.obstacles){
    if( (obs.center - theFieldBall.recentBallPositionOnField()).norm() < (nearest.translation - theFieldBall.recentBallPositionOnField()).norm() && obs.type == Obstacle::opponent) {
      nearest = obs.center;
    }
  }
  return nearest;
